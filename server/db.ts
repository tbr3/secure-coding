...
export function getPost(id: string, callback: (r: Post) => void) {
    return db.query(`SELECT * FROM ${postsTable} WHERE id="${id}"`, (err: any, result: any) => {
        if (err)
            throw err;
        callback(result);
    });
...
export function addComment(postId: string, content: string, name: string, callback: (r: any) => void) {
    return db.query(`INSERT INTO ${commentsTable} (postId, content, name) VALUES ("${postId}", "${content}", "${name}")`, (err: any, result: any) => {
        if (err)
            throw err;
        callback(result);
    });
